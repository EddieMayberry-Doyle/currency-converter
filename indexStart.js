...
class Bookmarker
{
    constructor()
    {
        ...
        this.appId = '0fae9b9f-e644-4574-bf2f-10e8f7ee994c';

        this.fillBookmarksList();

        this.addBookmark = this.addBookmark.bind(this);
        this.addEventHandlers = this.addEventHandlers.bind(this);
        document.getElementById("addButton").onclick = this.addBookmark;
    }

    addBookmark(event){
        event.preventDefault();
        const linkForHref = document.getElementById("url").value;
        const link = encodeURIComponent(linkForHref);
        const desc = document.getElementById("description").value;
        fetch(`${this.apiUrl}/${link}?app_id=${this.appId}`)
